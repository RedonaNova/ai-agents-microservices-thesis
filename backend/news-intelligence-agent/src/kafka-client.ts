...
class KafkaClient {
  ...
  constructor() {
    this.kafka = new Kafka({
      clientId: 'news-intelligence-agent',
      brokers: [process.env.KAFKA_BROKER || 'localhost:9092'],
      retry: { initialRetryTime: 100, retries: 8 }
    });
    this.consumer = this.kafka.consumer({ groupId: 'news-intelligence-group' });
  ...
  async subscribe(): Promise<void> {
    await this.consumer.subscribe({ topics: ['news-events'], fromBeginning: false });
    logger.info('Subscribed to news-events topic');
  }
