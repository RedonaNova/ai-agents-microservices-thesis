...
import { WELCOME_EMAIL_TEMPLATE, fillTemplate } from './email-templates';
...
async function generatePersonalizedIntro(event: UserRegistrationEvent): Promise<string> {
  try {
    ...
    logger.info('Generated personalized intro', { 
      email: event.email, 
      length: text.length 
    });
    ...
    logger.error('Failed to generate personalized intro', { error, email: event.email });
    // Fallback intro
...
export async function sendWelcomeEmail(event: UserRegistrationEvent): Promise<EmailResponse> {
  try {
    logger.info('Processing welcome email', { email: event.email, name: event.name });

    // Generate personalized intro
    const personalizedIntro = await generatePersonalizedIntro(event);

    // Fill email template
    const html = fillTemplate(WELCOME_EMAIL_TEMPLATE, {
      name: event.name,
    ...
    logger.error('Failed to send welcome email', { error, email: event.email });
    return {
      success: false,
...
async function fetchNewsForSymbols(symbols: string[]): Promise<NewsArticle[]> {
  ...
  try {
    ...
    for (const symbol of symbols.slice(0, 5)) { // Limit to 5 symbols
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          articles.push(...response.data.slice(0, 3).map((a: any) => ({
            headline: a.headline,
            summary: a.summary || '',
            source: a.source,
            url: a.url,
        ...
        logger.warn('Failed to fetch news for symbol', { symbol });
      }
    }
    ...
    if (articles.length === 0) {
      ...
      if (response.data && Array.isArray(response.data)) {
        articles.push(...response.data.slice(0, 10).map((a: any) => ({
          headline: a.headline,
          summary: a.summary || '',
          source: a.source,
          url: a.url,
...
async function summarizeNews(articles: NewsArticle[]): Promise<string> {
  try {
    if (articles.length === 0) {
    ...
    logger.error('Failed to summarize news', { error });
...
export async function sendDailyNewsSummary(user: User, symbols: string[]): Promise<EmailResponse> {
  try {
    logger.info('Processing daily news for user', { email: user.email, symbols });
    ...
    // Fill template
    const html = fillTemplate(NEWS_SUMMARY_EMAIL_TEMPLATE, {
      name: user.name,
    ...
    logger.info('Daily news email sent', { 
      email: user.email, 
      messageId: info.messageId,
      articlesCount: articles.length
    });
    ...
    logger.error('Failed to send daily news', { error, email: user.email });
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
...
export async function runDailyNewsJob(): Promise<{ success: boolean; emailsSent: number; errors: string[] }> {
  const errors: string[] = [];
  let emailsSent = 0;
  ...
  try {
    ...
    logger.info('Found users for daily news', { count: users.length });
    ...
    for (const userData of users) {
      try {
        ...
        logger.error('Failed to process user', { error, userData });
      }
    }

    logger.info('Daily news job completed', { emailsSent, errors: errors.length });
    ...
    logger.error('Daily news job failed', { error });
    return {
      success: false,
      emailsSent,
